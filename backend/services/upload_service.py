...
def create_social_content_record(user_id: int, title: str, common_caption: str, hashtags: str,
                                 ...
                                 platforms: list = None, original_timezone: str = "UTC") -> dict:
    """
    ...
    try:
        ...
        """, (
            user_id, title.strip(), common_caption or "", hashtags or "", media_meta.get("media_type", "VIDEO"),
            media_filename, thumb_filename, media_meta.get("file_size_bytes"),
            media_meta.get("duration_seconds"), media_meta.get("width"), media_meta.get("height"),
            media_meta.get("aspect_ratio"), validated_tz, expires_at
        ))
        ...
        for p in validated_platforms:
            cur.execute("""
                INSERT INTO social_content_platforms (
                    ...
                    custom_description, privacy_status, platform_status, processing_status
                )
                VALUES (%s, %s, %s, %s, %s, %s, %s, 'PENDING', 'IDLE')
...
def cleanup_content_media(content_id: int, user_id: int = None, force: bool = False) -> dict:
    ...
    try:
        ...
        media_del = safe_delete_temp_file(media_file) if media_file else {"removed": False, "status": "missing"}
        thumb_del = safe_delete_temp_file(thumb_file) if thumb_file else {"removed": False, "status": "missing"}
        
        # If physical deletion failed and file still exists on disk, do not clear DB reference
        if media_del.get("status") == "failed" or thumb_del.get("status") == "failed":
            return {
                "success": False,
                "message": "Failed to remove physical file from temporary storage. Reference preserved for retry.",
                "deleted": False
            }
        ...
        return {
            "success": True,
            "message": "Temporary media files deleted successfully.",
            "deleted": True
        }
    except Exception as e:
        if conn:
